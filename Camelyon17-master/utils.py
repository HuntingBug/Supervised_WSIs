...
#calculate the statistic parameters of outputs
...
def make_dir(slide_num, flags):
    ...
    if flags == 'slide':
        return cf.slide_path + 'A' + str(slide_num) + '.svs'

    elif flags == 'xml':
        return cf.xml_path + 'A' + str(slide_num) +'.xml'

    elif flags == 'mask':
        return cf.mask_path

    elif flags == 'map':
        return cf.mask_path +'A' + str(slide_num) + '_map.png'

    elif flags == 'tumor_mask':
        return cf.mask_path  + 'A'+str(slide_num) + '_tumor_mask.png'

    elif flags == 'tumor_patch':
        return cf.patch_path  + 'A'+str(slide_num) + '/tumor/'

    elif flags == 'normal_mask':
        return cf.mask_path + 'A' + str(slide_num) + '_normal_mask.png'

    elif flags == 'normal_patch':
        return cf.patch_path + 'A' + str(slide_num) + '/normal/'
    
    elif flags == 'tissue_mask':
        return cf.mask_path + 'A' + str(slide_num) + '_tissue_mask.png'
...
#check whether file is existed in filedir or not
...
# get the tumor, tissue, normal mask in specified mask level
def make_mask(slide_num, mask_level):
    ...
    slide_map = np.array(slide.get_thumbnail(slide.level_dimensions[hp.map_level]))  # map level = 4

    # xml loading
    coors_list = read_xml(slide_num, mask_level)  # coordinates of tumor region in mask level
    ...
    tumor_mask_exist = chk_file(mask_folder_path, 'b' + str(slide_num) + '_tumor_mask.png')  # check file exist or not
    if tumor_mask_exist == False:
        tumor_mask = np.zeros(slide.level_dimensions[mask_level][::-1])  # get the size of the mask based on mask level
        for coors in coors_list:
            cv2.drawContours(tumor_mask, np.array([coors]), -1, 255,
                             -1)  # draw the contours of tumor in mask level in based on the tumor mask
    ...
    if tissue_mask_exist == False:
        slide_lv = slide.read_region((0, 0), mask_level,
                                     slide.level_dimensions[mask_level])  # get slide image in mask level
        slide_lv = cv2.cvtColor(np.array(slide_lv), cv2.COLOR_RGBA2RGB)  # transfer image from RGBA to RGB
        slide_lv = cv2.cvtColor(slide_lv, cv2.COLOR_BGR2HSV)  # transfer image from BGR to HSV
        slide_lv = slide_lv[:, :, 1]  # get the saturation channel of image
        _, tissue_mask = cv2.threshold(slide_lv, 0, 255,
                                       cv2.THRESH_BINARY + cv2.THRESH_OTSU)  # get tissue mask in saturation channel using binary and otsu threshold
        cv2.imwrite(tissue_mask_path, np.array(tissue_mask))  # save mask
    ...
    if normal_mask_exist == False:
        tumor_mask = cv2.imread(tumor_mask_path, 0)  # copy tumor mask in mask level
        height, width = np.array(tumor_mask).shape
        for i in range(width):
            for j in range(height):
                if tumor_mask[j][i] > 127:
                    tissue_mask[j][
                        i] = 0  # if the tumor mask is large than half of 255, let the tissue mask to 0, remainning is normal region
        normal_mask = np.array(tissue_mask)
        cv2.imwrite(normal_mask_path, normal_mask)

# make normal and tissue patches from slides in specified mask level
def make_patch(slide_num, mask_level):
    ...
    slide_path = make_dir(slide_num,'slide') # slide storage path
    map_path = make_dir(slide_num, 'map') # map storage path
    mask_folder_path = make_dir(slide_num, 'mask') # mask path
    tumor_mask_path = make_dir(slide_num,'tumor_mask') # tumor mask path
    tumor_patch_path = make_dir(slide_num,'tumor_patch') # tumor path path
    normal_mask_path = make_dir(slide_num, 'normal_mask') # normal mask path
    normal_patch_path = make_dir(slide_num,'normal_patch') # normal path path
    ...
    tumor_mask_exist = chk_file(mask_folder_path, 'A'+str(slide_num)+'_tumor_mask.png')
    normal_mask_exist = chk_file(mask_folder_path, 'A'+str(slide_num)+'_normal_mask.png')
    ...
    p_size = hp.patch_size # patch size in true level
    width, height = np.array(slide.level_dimensions[0])//p_size #calculate the number of patches
    total = width * height # the total number of patches
    all_cnt = 0
    t_cnt = 0
    n_cnt = 0
    t_over = False
    n_over = False
    step = int(p_size/(2**mask_level)) #the size of patch in mask level

    for i in range(width):
        for j in range(height):
            ran = random.random()
            tumor_mask_sum = tumor_mask[step * j : step * (j+1),
                                            step * i : step * (i+1)].sum() #The sum of label on tumor mask in mask level of patch
            normal_mask_sum = normal_mask[step * j : step * (j+1),
                                            step * i : step * (i+1)].sum() #The sum of label on normal mask in mask level of patch
            mask_max = step * step * 255 #the maximum of sum
            tumor_area_ratio = tumor_mask_sum / mask_max #the ratio of tumor of patch in mask level
            normal_area_ratio = normal_mask_sum / mask_max # the ratio of normal of patch in mask level
            ...
            if (tumor_area_ratio > tumor_threshold) and (ran <= tumor_sel_ratio) and not t_over:
                ...
                patch = slide.read_region((p_size*i,p_size*j), 0, (p_size,p_size)) #extract tumor patch in level 0
                patch.save(patch_name)
                cv2.rectangle(slide_map, (step*i,step*j), (step*(i+1),step*(j+1)), (0,0,255), 1) #Add extraction label on slide image
                t_cnt += 1 #counting the tumor patch
                t_over = (t_cnt > tumor_sel_max) #judge the number of tumor patch
            ...
            # extract normal patch
                ...
                patch = slide.read_region((p_size*i,p_size*j), 0, (p_size,p_size)) #extract normal patch in level 0
                patch.save(patch_name)
...
#devide patches into trainset, validation set and test set
# if flags is train, move all files into training set
# if flags is not train, move half of files into test set, another half of files into validation set
def divide_patch(slide_num, flags):
    ...
    tumor_num = len(tumor_files) #the number of tumor path
    ...
    if flags == 'train':
        for i in range(tumor_num):
            shutil.move(tumor_files[i],cf.dataset_path+'train/') #move all tumor files into trainset

        os.chdir(normal_patch_path)

        for i in range(normal_num):
            shutil.move(normal_files[i], cf.dataset_path+'train/') #move all normal files into train set
         
    else:
        for i in range(tumor_num):
            if i%2 == 0:
                shutil.move(tumor_files[i], cf.dataset_path+'validation/') #move tumor files into validation set if i is even number
            else:
                shutil.move(tumor_files[i], cf.dataset_path+'test/') # move tumor files into test set if i is odd number
        
        os.chdir(normal_patch_path)

        for i in range(normal_num):
            if i%2 == 0:
                shutil.move(normal_files[i], cf.dataset_path+'validation/') #move tumor files into validation set if i is even number
            else:
                shutil.move(normal_files[i], cf.dataset_path+'test/') #move files into test set if i is odd number
        
    os.chdir(cf.origin_path)


#label files with t_ as 1, n_ as 0
#save the csv file
def make_label():
